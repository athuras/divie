...
def get_itemsJSON(userID):
    query = "SELECT item.item_id, item.name, item.description, item.img_url, coalesce(bid.value, 0) as value FROM item LEFT JOIN bid ON item.item_id = bid.item_id WHERE bid.agent_id = " + userID + ";"
