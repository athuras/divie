...
function addAssets()
{
	...
	for(var i=0; i<AssetList.length; i++)
	{
		...
		// if ranks have already been assigned load those instead
		var hasRank = false;
		if (AssetList[i].rank > 0)
		{
			newValueBarDiv.style.width = (parseInt(AssetList[i].rank) / MAX_BUDGET_VALUE * 100)  + "%";
			var newRemIndicator = document.createElement("div");
			newRemIndicator.setAttribute("id", "remIndicator");
		...
		if (hasRank)
		{
			newLi.appendChild(newRemIndicator);
		}
		else
		{
